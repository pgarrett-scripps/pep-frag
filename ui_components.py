...
def setup_sidebar() -> Dict[str, Any]:
    with st.sidebar:
        ...
        c2.image('images/yateslogo.png', use_container_width=True)
        ...
        **Peptide Sequence**: Enter the peptide sequence to fragment. Include modifications in square brackets.
        ...
        if condense_to_mass_notation:
            peptide_sequence = pt.condense_to_mass_mods(peptide_sequence, include_plus=True, precision=precision)
...
def setup_fragment_checkboxes():
    ...
    with c1:
        a = stp.checkbox('a', value='a' in DEFAULT_FRAGMENT_TYPES, key='a')
        x = stp.checkbox('x', value='x' in DEFAULT_FRAGMENT_TYPES, key='x')

    with c2:
        b = stp.checkbox('b', value='b' in DEFAULT_FRAGMENT_TYPES, key='b')
        y = stp.checkbox('y', value='y' in DEFAULT_FRAGMENT_TYPES, key='y')

    with c3:
        c = stp.checkbox('c', value='c' in DEFAULT_FRAGMENT_TYPES, key='c')
        z = stp.checkbox('z', value='z' in DEFAULT_FRAGMENT_TYPES, key='z')
...
def setup_nterm_fragment_pills():
    """Setup fragment pill options"""
    fragment_pills = stp.segmented_control('N-Term Fragments',
                              selection_mode='multi',
                              options=list('abc'),
...
def setup_cterm_fragment_pills():
    """Setup fragment pill options"""
    fragment_pills = stp.segmented_control('C-Term Fragments',
                              selection_mode='multi',
                              options=list('xyz'),
